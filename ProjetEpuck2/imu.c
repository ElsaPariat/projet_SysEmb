...
void imu_compute_units(void){
	...
	//acc est sur +/-2g à traduire en m/s^2
	imu_values.acceleration[i] = ( (imu_values.acc_raw[i] - imu_values.acc_offset[i]) * STANDARD_GRAVITY * ACC_RAW2G);
	//gyro est sur +/-250 degrees/s à traduire en rad/s
	imu_values.gyro_rate[i] = ( (imu_values.gyro_raw[i] - imu_values.gyro_offset[i]) * DEG2RAD(GYRO_RAW2DPS) );
	}
...
void imu_compute_offset(messagebus_topic_t * imu_topic, uint16_t nb_samples){
  ...
  uint16_t i= 0;
  imu_values.acc_offset[X_AXIS] = 0;
  imu_values.acc_offset[Y_AXIS] = 0;
  imu_values.acc_offset[Z_AXIS] = 0;
  imu_values.gyro_offset[X_AXIS] = 0;
  imu_values.gyro_offset[Y_AXIS] = 0;
  imu_values.gyro_offset[Z_AXIS] = 0;
  while (i < nb_samples){
    //wait for new measures to be published
    messagebus_topic_wait(imu_topic, &imu_values, sizeof(imu_values));
    i++;
    imu_values.acc_offset[X_AXIS] += imu_values.acc_raw[X_AXIS];
    imu_values.acc_offset[Y_AXIS] += imu_values.acc_raw[Y_AXIS];
    imu_values.acc_offset[Z_AXIS] += imu_values.acc_raw[Z_AXIS];
    imu_values.gyro_offset[X_AXIS]+= imu_values.gyro_raw[X_AXIS];
    imu_values.gyro_offset[Y_AXIS]+= imu_values.gyro_raw[Y_AXIS];
    imu_values.gyro_offset[Z_AXIS]+= imu_values.gyro_raw[Z_AXIS];
    }
  imu_values.acc_offset[X_AXIS]/= nb_samples;
  imu_values.acc_offset[Y_AXIS]/= nb_samples;
  imu_values.acc_offset[Z_AXIS]/= nb_samples;
  imu_values.gyro_offset[X_AXIS]/=nb_samples;
  imu_values.gyro_offset[Y_AXIS]/=nb_samples;
  imu_values.gyro_offset[Z_AXIS]/=nb_samples;
